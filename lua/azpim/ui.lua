...
function render()
	...
	local who = state.account
			and string.format(
				"%s  ·  %s",
				state.account.user and state.account.user.name or "?",
				state.account.tenantDisplayName or state.account.tenantId or ""
			)
		or "…"
	add("Azure PIM   " .. who, "AzPimTitle")
	add("<Tab> select   <CR> activate   a activate selected   d deactivate   r refresh   q quit", "AzPimHelp")
	add("")
...
local function with_opts(action, count, fn)
	...
	ask(
		string.format("Justification (%d role%s): Infrastructure work", count, count == 1 and "" or "s"),
		cfg.justification,
		function(just)
