...
local function curl(args, cb)
  ...
  local ok, err = pcall(vim.system, cmd, { text = true }, function(out)
    vim.schedule(function()
      ...
      if not decoded then
        return cb(nil, "could not parse Graph response as JSON: " .. vim.trim(out.stdout))
      end
      cb(data, nil)
    end)
  end)
...
--- Percent-encode a URL's query string. Graph rejects the raw spaces and
--- quotes that `$filter=principalId eq '<oid>'` would otherwise send.
local function encode_query(url)
